...
class Point {
  ...
  getIntersectLine(range: number, row: number): Line | undefined {
    ...
    let [x1, x2] = [this.x - dx, this.x + dx]
    if (x2 < 0) return undefined
    if (x1 > rowMax) return undefined
    if (x1 < 0) x1 = 0
    if (x2 > rowMax) x2 = rowMax
...
const main = (filename: string) => {
  ...
  const getRowCount = (desiredRow: number) => {
      ...
      while (thisLineIndex < lineSegments.length - 1) {
        ...
        while (nextLineIndex < lineSegments.length) {
          ...
          if (nextLine.x1 > thisLine.x2) {
            // console.log(`no overlap between ${thisLine.x2} and ${nextLine.x1}`)
            xAnswer = thisLine.x2 + 1
          ...
          if (nextLine.x2 <= thisLine.x2) {
            // console.log('complete overlap, ignore and move next line forwards')
            nextLineIndex++
            continue
          }
  ...
  rowMax = sensors.length === 35 ? 4000000 : 20

  let answer: number

  for (let rowNumber = 0; rowNumber <= rowMax; rowNumber++) {
    const count = getRowCount(rowNumber)
    // console.log({ rowNumber, count })
    if (count === rowMax) {
      // console.log({ x: xAnswer!, y: rowNumber })
      answer = xAnswer! * 4000000 + rowNumber
      break
    }
  }
