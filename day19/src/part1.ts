...
const executeMinutes = (state: State, blueprint: Blueprint): number => {
  if (state.minuteNumber > state.maxMinutes) {
    ...
    if (result === 21) {
      console.log('returning 21', { state })
    }
  ...
  if (
    ...
    if (result === 21) {
      console.log('returning 21 after buying geode robot')
    }
    return result
  }
...
const main = () => {
  ...
  let totalQuality = 0
  let blueprintNumber = 1

  for (let blueprint of blueprints) {
    console.log({ blueprintNumber })
    cache = new Map<string, number>()
    totalQuality += executeMinutes(initialState, blueprint) * blueprintNumber
    blueprintNumber++
  }
